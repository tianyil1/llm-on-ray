...
if args.streaming_response:
    output = llm.stream(prompt.format(text="sports that don't use balls"))
    for chunk in output:
        print(chunk.content, end="", flush=True)
else:
    output = llm.predict(text=prompt.format(voices="BDL", text="sports that don't use balls"))
    output = np.fromstring(output.replace('[', '').replace(']', ''), sep=' ').astype(np.int16)
